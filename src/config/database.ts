...
// Tipo práctico para la instancia (forzamos 'any' para permitir '$use' sin error de TS)
type AnyPrismaClient = PrismaClient | any;

// ----- Creación / Singleton para hot-reload -----
declare global {
  // eslint-disable-next-line no-var
  var prisma_global: AnyPrismaClient | undefined;
}

const globalForPrisma = globalThis as unknown as { prisma_global?: AnyPrismaClient };
...
function createPrismaClient(): AnyPrismaClient {
  ...
  const client: AnyPrismaClient = new PrismaClient({ 
...
export const prisma: AnyPrismaClient = globalForPrisma.prisma_global ?? createPrismaClient();
if (process.env.NODE_ENV !== 'production') globalForPrisma.prisma_global = prisma;
...
export async function healthCheck(): Promise<{ status: 'healthy' | 'unhealthy'; responseTime: number; timestamp: string; database: string }> {
  const start = Date.now();
  try {
