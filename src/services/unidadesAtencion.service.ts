...
import type { UnidadAtencionInput } from '../schemas/unidadesAtencion.schema.js';

// Importamos tipos desde el paquete estándar
import type { Prisma, UnidadAtencion as PrismaUnidadAtencion } from '@prisma/client';

// Alias de tipo para simplificar las cláusulas where
type UnidadAtencionWhereInput = Prisma.UnidadAtencionWhereInput;
type UnidadAtencionUpdateInput = Prisma.UnidadAtencionUpdateInput; // 🟢 NUEVO TIPO: Para manejar los datos de update
...
export class UnidadesAtencionService {
	...
	async getAll(filters: { tipo?: string; estado?: string; page?: number; limit?: number }) {
		...
		const where: UnidadAtencionWhereInput = {}; 
	...
	async getById(id: number): Promise<PrismaUnidadAtencion> {
	...
	async update(id: number, data: Partial<UnidadAtencionInput>) {
		...
		const updateData: UnidadAtencionUpdateInput = { ...data };
