...
export class PersonasAtendidasService {
  ...
  async create(data: PersonaAtendidaInput) {
  ...
  async update(id: number, data: UpdatePersonaInput) {
...
// Si se actualiza el documento, verificar que no exista
  ...
  async getEstadisticas(id: number) {
    ...
    const [citasTotal, citasCompletadas, episodiosTotal, episodiosAbiertos] =
      await Promise.all([
