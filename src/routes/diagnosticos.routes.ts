...
router.get(
    ...
    validate(z.object({ params: z.object({ episodioId: idParamSchema }) })),
    getAllDiagnosticosByEpisodio
);
...
router.post(
    '/',
    checkAuth, 
    validate(createDiagnosticoSchema), // Valida el cuerpo (body) que incluye episodioId, codigo, etc.
    createDiagnostico
);
...
router.get(
    '/:id',
    checkAuth, 
    validate(z.object({ params: z.object({ id: idParamSchema }) })),
    getDiagnosticoById
);
...
router.put(
    '/:id',
    checkAuth,
    validate(updateDiagnosticoSchema), // Valida tanto el ID del parámetro como el cuerpo parcial
    updateDiagnostico
);
