...
router.get(
    ...
    validate(z.object({ params: z.object({ personaId: idParamSchema }) })),
    getAllConsentimientosByPersona
);
...
router.post(
    '/',
    checkAuth, 
    validate(createConsentimientoSchema), 
    createConsentimiento
);
...
router.put(
    '/:id',
    checkAuth,
    validate(updateConsentimientoSchema), // Valida el ID y el cuerpo parcial
    updateConsentimiento
);
